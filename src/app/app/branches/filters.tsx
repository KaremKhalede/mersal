...
export function BranchFilters({
  ...
  // Debounced live search — no submit button, matches the rest of this filter row auto-applying.
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-2">
        <select value={status ?? ""} onChange={(e) => update({ status: e.target.value || undefined })} className={SELECT_CLASS}>
        ...
        <div className="relative">
          ...
          <select value={country ?? ""} onChange={(e) => update({ country: e.target.value || undefined })} className={cn(SELECT_CLASS, "ps-7")}>
