...
import { MapPin, Boxes, Printer } from "lucide-react";
      ...
      <div className="space-y-3">
        {await Promise.all(
          trip.stops.map(async (stop) => {
            ...
            return (
              <Card key={stop.id} data-testid={`stop-${stop.id}`}>
                <CardHeader className="flex-row items-center justify-between space-y-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary text-primary-foreground text-xs font-bold">{stop.sequence}</span>
                  ...
                  <div className="flex items-center gap-2">
                    {stop.loadingEnabled && <AssignShipmentDialog tripId={trip.id} shipments={eligible} />}
                    <StopActions
                      tripId={trip.id}
                      stopId={stop.id}
                ...
                <CardContent className="grid sm:grid-cols-2 gap-4">
                  {stop.loadingEnabled && (
                    <div className="rounded-lg border p-3">
                      ...
                      {pendingLoad > 0 && <p className="text-xs text-warning mt-1">بانتظار تحميل {pendingLoad} شحنة ({cartonsToLoad} كرتون)</p>}
                    </div>
                  )}
                  {stop.unloadingEnabled && (
                    <div className="rounded-lg border p-3">
                      ...
                      {pendingUnload > 0 && <p className="text-xs text-warning mt-1">بانتظار تفريغ {pendingUnload} شحنة ({cartonsToUnload} كرتون)</p>}
                    </div>
                  )}
