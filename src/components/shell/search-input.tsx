...
export function SearchInput({ defaultValue, placeholder }: { defaultValue?: string; placeholder: string }) {
  ...
  return (
    <div className="relative w-full">
      ...
      />
      <kbd className="absolute start-2.5 top-1/2 -translate-y-1/2 hidden sm:flex h-5 items-center rounded border bg-background px-1.5 text-2xs text-muted-foreground">
        /
      </kbd>
    </div>
  );
