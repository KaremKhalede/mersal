...
export default function ProductShowcase() {
  ...
  return (
    <section id="about" ref={ref} className="chargee-section bg-white overflow-hidden">
      <div className="chargee-container">
        <div className="grid lg:grid-cols-2 gap-[var(--grid-gap)] items-center">
          <div className="order-2 lg:order-1">
            <Reveal><span className="pill-badge">لوحة التحكم</span></Reveal>
            <Reveal index={1}>
              <h2 className="chargee-section-title mt-5">
                من الصورة الكاملة إلى <span className="chargee-accent">أدق التفاصيل</span>
              </h2>
            </Reveal>
            <Reveal index={2}>
            ...
            <Reveal index={3} className="mt-8 grid grid-cols-2 gap-4 max-w-md">
              {[
                ...
                <div key={s.label} className="rounded-xl border border-[var(--chargee-border)] bg-[var(--chargee-bg)] p-4">
                  <p className="text-xs text-[var(--chargee-muted)]">{s.label}</p>
                  <p className="mt-1 font-bold text-[var(--chargee-text)] text-sm">{s.value}</p>
                </div>
              ))}
            </Reveal>
          </div>
